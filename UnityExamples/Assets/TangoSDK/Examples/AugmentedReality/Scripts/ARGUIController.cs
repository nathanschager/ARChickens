...
public class ARGUIController : MonoBehaviour, ITangoLifecycle, ITangoDepth
{
    ...
    /// <summary>
    /// The logo prefab to place on taps.
    /// </summary>
    public GameObject m_prefabLogo;
    ...
    /// <summary>
    /// If set, this is the selected logo.
    /// </summary>
    private ARLogo m_selectedLogo;
    ...
    /// <summary>
    /// If set, this is the rectangle to toggle placing the chickens.
    /// </summary>
    private Rect m_placeChicken;

    /// <summary>
    /// If set, this is the rectangle to toggle placing our logo.
    /// </summary>
    private Rect m_placeLogo;
    ...
    public void OnGUI()
    {
    	m_placeLogo = new Rect(Screen.width - 2*(UI_BUTTON_SIZE_X - UI_BUTTON_GAP_X),
                                     Screen.height - UI_BUTTON_SIZE_Y - UI_BUTTON_GAP_X,
                                     UI_BUTTON_SIZE_X,
                                     UI_BUTTON_SIZE_Y);

        m_placeChicken = new Rect(Screen.width - 3*(UI_BUTTON_SIZE_X - UI_BUTTON_GAP_X),
                                     Screen.height - UI_BUTTON_SIZE_Y - UI_BUTTON_GAP_X,
                                     UI_BUTTON_SIZE_X,
                                     UI_BUTTON_SIZE_Y);
        ...
        if (GUI.Button(m_placeLogo, "<size=30>Place Logo</size>")){
        	logoToggle = true;

        }else if(GUI.Button(m_placeChicken, "<size=30>Place Chickens</size>")){
        	logoToggle = false;
        }
    ...
    private IEnumerator _WaitForDepthAndFindPlane(Vector2 touchPosition)
    {
        ...
        // this is where we switch what object is being placed. 
        // tp = new Transform;
        // tp.position = Vector3(0, 0, 0);
        GameObject m_prefab;
        if (!logoToggle){ //place chicken
        	m_prefab = m_prefabMarker;
		}else{ //place logo
			m_prefab = m_prefabLogo;
		}
		m_prefab.transform.parent = null;
	    Instantiate(m_prefab, planeCenter, Quaternion.LookRotation(forward, up));
	    m_selectedMarker = null;
    }
}
